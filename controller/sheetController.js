...
exports.getDummySheet = catchAsynch(async (req, res, next) => {
  ...
  const {mainHeading,address,reportName,fromDate,toDate,parameter,reportHeading,records}= data[reportNumber].report
 ...
 const galanfi = worksheet.addImage({
  ...
  worksheet.cell(1, 4, 1, 6, true).string(mainHeading).style(mainHeadingStyle);
  ...
  worksheet.cell(4, 4).string("FROM DATE").style(style_srNo);  worksheet.cell(4, 5,4,6,true).string(fromDate).style(style_srNo);
  worksheet.cell(5, 4).string("TO DATE").style(style_srNo); worksheet.cell(5, 5,5,6,true).string(toDate).style(style_srNo);
  worksheet.cell(6, 4).string("PARAMETER").style(style_srNo); worksheet.cell(6, 5,6,6,true).string(parameter).style(style_srNo);

  worksheet.cell(7, 4,7,6,true).string("").style({fill: {
    type: "pattern",
    patternType: "solid",
    fgColor: "#00b0f0",
  },
border:groupBorder});
  ...
  //makeing columns
  worksheet.cell(startrow, 1, startrow+1, 1, true).string("SR. NO.").style(colHeader);
  worksheet.cell(startrow, 2, startrow+1, 2, true).string("GROUP NAME").style(colHeader);
  worksheet.cell(startrow, 3, startrow+1, 3, true).string("SUB GROUP").style(colHeader);
  worksheet.cell(startrow, 4, startrow+1, 4, true).string("SUB GROUP 1").style(colHeader);
  worksheet.cell(startrow, 5, startrow+1, 5, true).string("SUB GROUP 2").style(colHeader );
  worksheet.cell(startrow, 6, startrow+1, 6, true).string("METERS").style(colHeader);
  worksheet.cell(startrow, 7, startrow+1, 7, true).string("KWH IMP").style(colHeader);
  worksheet.cell(startrow, 8, startrow+1, 8, true).string("KWH EXP").style(colHeader);

  worksheet.cell(startrow, 9,startrow,10,true).string("GROUP TOTAL").style(colHeader);

  worksheet.cell(startrow+1, 9).string("KWH IMPORT").style(colHeader);
  worksheet.cell(startrow+1, 10).string("KWH EXPORT").style(colHeader);


  worksheet.cell( startrow+1,11).string("METER SID")
  .style(idesStyle);
  worksheet.cell(startrow+1,12).string("DEVICE IMEI")
  .style(idesStyle);
